...
// 67. 智能场景启用工具
server.tool("smart_enable_scenario", "根据用户意图智能启用相关脚本组合。例如：'加密分析'会自动启用Hook JSEncrypt RSA和Hook CryptoJS。对于'未授权测试'会先检测Vue站点。", {
...
// 68. 获取可用场景列表
server.tool("list_scenarios", "获取所有可用的智能场景配置列表", {}, async () => {
...
server.tool("quick_encryption_analysis", "一键启用加密分析：自动开启 Hook JSEncrypt RSA 和 Hook CryptoJS，然后分析页面加密方式", {
...
server.tool("quick_anti_debug_bypass", "一键反调试绕过：自动检测页面反调试机制并启用相应的绕过脚本", {
    autoRefresh: z.boolean().optional().describe("是否自动刷新页面，默认true")
}, async ({ autoRefresh = true }) => {
    try {
        const results = { steps: [] };
...
server.tool("quick_api_analysis", "一键API分析：启用XHR/Fetch Hook脚本，捕获并分析API请求", {
    autoRefresh: z.boolean().optional().describe("是否自动刷新页面，默认true")
}, async ({ autoRefresh = true }) => {
    try {
        const results = { steps: [] };
...
server.tool("quick_vue_analysis", "一键Vue分析：启用路由获取脚本，获取路由信息并可选择清除路由守卫", {
...
server.tool("smart_unauthorized_test", "智能未授权测试：先检测是否为Vue站点，如果是则进行大屏未授权测试和Vue路由未授权测试；如果不是Vue站点，则提示只能进行通用分析（如加密解密）", {
...
server.tool("detect_site_framework", "检测当前站点使用的前端框架（Vue、React等），用于判断可以使用哪些功能", {}, async () => {
    try {
        const results = {
            frameworks: {
                vue: false,
...
// 75. 批量启用脚本
server.tool("batch_enable_scripts", "批量启用或禁用多个脚本", {
...
// 74. 禁用所有脚本
server.tool("disable_all_scripts", "禁用当前页面的所有已启用脚本", {
    autoRefresh: z.boolean().optional().describe("是否自动刷新页面，默认false")
