...
// ============== 工具定义 ==============
// 1. 获取连接状态
server.tool("get_connection_status", "获取浏览器扩展的连接状态", {}, async () => {
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
...
// 2. 获取当前页面信息
server.tool("get_page_info", "获取当前浏览器页面的基本信息（URL、标题、域名等）", {}, async () => {
    try {
...
// 3. 获取网络请求列表
server.tool("get_network_requests", "获取页面的网络请求/API调用记录", {
...
// 4. 获取Vue Router信息
server.tool("get_vue_routes", "获取页面的Vue Router路由信息（需要先启用获取路由脚本）", {}, async () => {
    try {
...
// 5. 获取Hook捕获的数据
server.tool("get_hook_data", "获取Hook脚本捕获的数据（cookie、localStorage、XHR等）", {
...
// 6. 获取已启用的脚本列表
server.tool("get_enabled_scripts", "获取当前页面已启用的脚本列表", {}, async () => {
    try {
...
// 7. 启用/禁用脚本
server.tool("toggle_script", "启用或禁用指定的脚本", {
...
// 8. 导航到指定URL
server.tool("navigate_to", "导航到指定的URL或Vue路由", {
...
// 9. 获取Cookie
server.tool("get_cookies", "获取当前页面的Cookie", {
...
// 10. 获取LocalStorage
server.tool("get_local_storage", "获取当前页面的LocalStorage数据", {
...
// 11. 获取SessionStorage
server.tool("get_session_storage", "获取当前页面的SessionStorage数据", {
    key: z.string().optional().describe("键名过滤")
}, async ({ key }) => {
    try {
...
// 12. 执行JavaScript代码
server.tool("execute_script", "在页面上下文中执行JavaScript代码", {
...
// 13. 获取DOM元素信息
server.tool("get_dom_info", "获取页面DOM结构信息", {
...
// 14. 配置Hook脚本参数
server.tool("configure_hook", "配置Hook脚本的参数（如关键字过滤、debugger开关等）", {
...
// 15. 获取可用脚本列表
server.tool("list_available_scripts", "获取所有可用的脚本列表及其描述", {
...
// 16. 刷新页面
server.tool("refresh_page", "刷新当前页面", {
...
// ============== 新增：浏览器控制功能 ==============
// 17. 截图
server.tool("take_screenshot", "截取当前页面的屏幕截图", {
...
// 18. 点击元素
server.tool("click_element", "点击页面上的元素", {
...
// 19. 填充输入框
server.tool("fill_input", "在输入框中填充文本", {
...
// 21. 获取控制台消息
server.tool("get_console_messages", "获取浏览器控制台消息", {
...
// 22. 滚动页面
server.tool("scroll_page", "滚动页面到指定位置或元素", {
...
// 23. 等待选择器
server.tool("wait_for_selector", "等待页面上出现指定元素", {
...
// 24. 获取元素信息
server.tool("get_element_info", "获取页面元素的详细信息", {
    selector: z.string().describe("CSS选择器")
}, async ({ selector }) => {
    try {
...
// 25. 准备路由访问（自动启用必要脚本）
server.tool("prepare_route_access", "准备访问Vue路由（自动启用清除路由守卫和清除跳转脚本）", {}, async () => {
    try {
...
// 26. 扫描路由收集API
server.tool("scan_route_for_api", "访问指定路由并收集该路由触发的API请求", {
...
// 27. 批量扫描路由收集API
server.tool("batch_scan_routes", "批量访问多个路由并收集所有API请求", {
...
// ============== 加密分析功能 ==============
// 28. 启用加密 Hook 脚本
server.tool("enable_encryption_hooks", "启用 RSA 和 CryptoJS 加密 Hook 脚本，用于捕获加密数据", {}, async () => {
    try {
...
// 29. 分析页面加密
server.tool("analyze_page_encryption", "分析当前页面使用的加密库和密钥", {}, async () => {
    try {
...
// 30. 自动登录并捕获加密数据
server.tool("auto_login_and_capture", "自动填写登录表单并捕获加密数据（密码加密前后的值）", {
...
// 31. 获取捕获的加密数据
server.tool("get_captured_encryption", "获取 Hook 脚本捕获的加密数据（公钥、明文、密文等）", {}, async () => {
    try {
...
// 32. RSA 解密
server.tool("decrypt_rsa", "使用私钥解密 RSA 加密的数据", {
...
// 33. 从JS文件提取密钥
server.tool("extract_keys_from_js", "从指定的JS文件中提取RSA公钥、私钥等加密信息", {
...
// 34. 一键分析登录加密（综合功能）
server.tool("analyze_login_encryption", "一键分析登录页面的密码加密方式：启用Hook -> 自动登录 -> 捕获加密数据 -> 提取密钥 -> 尝试解密", {
...
// ============== 智能化功能：反调试 ==============
// 35. 检测反调试机制
server.tool("detect_anti_debug", "检测页面中的反调试机制（debugger语句、控制台检测、窗口大小检测等）", {}, async () => {
    try {
...
// 36. 自动绕过反调试
server.tool("auto_bypass_anti_debug", "检测并自动启用相应的反调试绕过脚本", {}, async () => {
    try {
...
// ============== 智能化功能：API分析 ==============
// 37. 分析API签名
server.tool("analyze_api_signature", "分析最近的API请求，提取签名参数、加密参数等特征", {}, async () => {
    try {
...
// ============== 智能化功能：框架数据提取 ==============
// 38. 提取Vue数据
server.tool("extract_vue_data", "提取页面Vue组件的data、computed、methods等数据", {
...
// 39. 提取React数据
server.tool("extract_react_data", "提取页面React组件的props、state等数据", {
...
// ============== 智能化功能：认证分析 ==============
// 40. 分析认证机制
server.tool("analyze_authentication", "分析页面的认证机制（Cookie、localStorage中的token、JWT解析等）", {}, async () => {
    try {
...
// ============== 智能化功能：表单自动化 ==============
// 41. 获取页面表单
server.tool("get_page_forms", "获取页面所有表单信息（输入框、按钮、选择器等）", {}, async () => {
    try {
...
// 42. 自动填充表单
server.tool("auto_fill_form", "自动填充指定表单的输入框", {
...
// ============== 智能化功能：WebSocket监控 ==============
// 43. 注入WebSocket监控
server.tool("inject_ws_monitor", "注入WebSocket监控，捕获所有WebSocket通信", {}, async () => {
    try {
...
// 44. 获取WebSocket消息
server.tool("get_ws_messages", "获取捕获的WebSocket消息", {
...
// ============== 智能化功能：数据爬取 ==============
// 45. 提取页面数据
server.tool("extract_page_data", "根据选择器提取页面结构化数据", {
...
// 46. 提取表格数据
server.tool("extract_table_data", "提取页面表格数据为结构化格式", {
...
// ============== 综合功能：一键分析页面 ==============
// 47. 一键分析页面
server.tool("full_page_analysis", "一键全面分析页面：反调试检测、框架识别、加密分析、认证分析、表单识别", {}, async () => {
    try {
...
// ============== Burp Suite 格式网络请求 ==============
// 48. 点击元素并捕获请求
server.tool("click_and_capture", "点击指定元素并捕获产生的所有网络请求（包含请求头、响应头、响应体）", {
...
// 49. 获取 Burp 格式网络请求
server.tool("get_network_requests_burp", "获取网络请求并转换为Burp Suite格式（包含完整的HTTP请求和响应）", {
    filter: z.string().optional().describe("URL过滤关键词"),
...
// 50. 点击并获取 Burp 格式请求
server.tool("click_and_get_burp", "点击元素（如登录按钮）并获取产生的网络请求，输出Burp Suite格式", {
...
server.tool("login_and_get_burp", "自动填写登录表单、点击登录按钮，并获取登录请求的Burp格式数据包", {
    username: z.string().describe("用户名"),
    password: z.string().describe("密码"),
    usernameSelector: z.string().optional().describe("用户名输入框选择器"),
    passwordSelector: z.string().optional().describe("密码输入框选择器"),
    ...
    for (const [key, config] of Object.entries(SENSITIVE_PATTERNS)) {
        if (patternsToCheck.includes(key) || patternsToCheck.includes('all')) {
            ...
            if (matches && matches.length > 0) {
                ...
                if (key === 'chineseName') {
                    const filtered = uniqueMatches.filter(m => !['用户名', '密码', '登录', '注册', '确认', '取消', '提交', '返回', '首页', '管理'].includes(m));
...
// 52. 扫描 API 响应敏感数据
server.tool("scan_sensitive_data", "扫描最近的API响应中是否包含敏感数据（身份证、手机号、银行卡、密码等）", {
    filter: z.string().optional().describe("URL过滤关键词"),
...
// 53. 扫描路由敏感数据泄露
server.tool("scan_route_sensitive_data", "访问指定路由并扫描返回数据中的敏感信息泄露", {
...
// 54. 批量扫描多个路由的敏感数据
server.tool("batch_scan_sensitive_routes", "批量扫描多个路由，检测敏感数据泄露（适用于大屏、后台等未授权访问测试）", {
...
// 55. 实时监控敏感数据（开启监控模式）
server.tool("start_sensitive_monitor", "开始实时监控所有API响应中的敏感数据泄露", {
...
// 56. 获取敏感数据监控告警
server.tool("get_sensitive_alerts", "获取敏感数据监控产生的告警信息", {
